...
class BotRegex {
    ...
    private val parlamatch = Regex("^!parla (.*)$", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE, RegexOption.DOT_MATCHES_ALL))
    private val parlasuper = Regex("^!parlasuper (.*)$", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE, RegexOption.DOT_MATCHES_ALL))
    private val chiera = Regex("^!chiera$", setOf(RegexOption.IGNORE_CASE, RegexOption.MULTILINE, RegexOption.DOT_MATCHES_ALL))
    ...
    fun parolacceMatch(message: Message) = parolacce.containsMatchIn(message.text)
    fun sloganMatch(message: Message) = content(message, sloganmatch)
    fun parlaMatch(message: Message) = content(message, parlamatch)
    fun parlaSuper(message: Message) = content(message, parlasuper)
    fun chiEra(message: Message) = chiera.containsMatchIn(message.text)
    ...
    fun last(message: Message) = chiera.containsMatchIn(message.text)
