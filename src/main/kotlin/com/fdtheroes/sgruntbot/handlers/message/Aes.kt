...
@Service
class Aes(botUtils: BotUtils, botConfig: BotConfig) : MessageHandler(botUtils, botConfig), HasHalp {
    ...
    override fun handle(message: Message) {
        ...
        if (groupValues?.size == 4) {
            ...
            if (decrypt.isEmpty()) {
                botUtils.rispondi(ActionResponse.message(encrypt(key, body)), message)
            } else {
                botUtils.rispondi(ActionResponse.message(decrypt(key, body)), message)
            }
        }
    }
